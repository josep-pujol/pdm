...
import tweepy
from tweepy.models import Status as TweetObject
...
class TwitterClient:
    ...
    def __init__(self):
        ...
        self.auth: OAuthHandler = OAuthHandler(consumer_key, consumer_secret)
        self.auth.set_access_token(access_token, access_token_secret)
        self.api: TwitterClient = tweepy.API(self.auth)

    @staticmethod
    def parse_tweet(tweet: TweetObject, tweet_parser: Callable = None) -> Any:
    ...
    @staticmethod
    def remove_duplicate_tweets(tweets: TweetObject) -> Set[TweetObject]:
    ...
    def get_tweets(
        ...
        Returns:
            List of tweets
        """
        fetched_tweets: List[TweetObject] = self.fetch_tweets(
            query=query, count=count, geo=geo, lang=lang, until=until
        )
        unique_tweets: Set[TweetObject] = TwitterClient.remove_duplicate_tweets(fetched_tweets)
        return [
